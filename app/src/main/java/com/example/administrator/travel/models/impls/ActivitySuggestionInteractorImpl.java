...
public class ActivitySuggestionInteractorImpl implements ActivitySuggestionInteractor {
    ...
    @Override
    public void getActivitySuggestion(boolean isTourGuide, final Listener.OnGetActivitySuggestionsFinishedListener listener) {
        FirebaseDatabase database = FirebaseDatabase.getInstance();
        DatabaseReference suggestsRef = database.getReference(SUGGEST_ACTIVITIES_REF);
        Query getSuggestByRoleQuery = suggestsRef.orderByChild("forUser").equalTo(!isTourGuide);
        getSuggestByRoleQuery.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
